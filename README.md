...
**Сначала — где взять сам ключ:**
- Напрямую: https://thejevai.com → регистрация/вход → раздел с API-ключами (обычно "API keys"/"Developers" в настройках аккаунта) → создать ключ → скопировать значение.
- Через OpenRouter: https://openrouter.ai/keys → войти → "Create Key" → скопировать (формат `sk-or-v1-...`).

Держите окно с ключом открытым — обычно он показывается один раз, копируйте его сразу в буфер обмена.

**Способ 1: через графический интерфейс Windows (рекомендуется)**

1. Нажмите клавишу **Win**, начните печатать: `переменные среды` (или `environment variables`).
2. В результатах кликните **"Изменение переменных среды для вашей учётной записи"** (англ. *"Edit environment variables for your account"*) — не вариант "для всех пользователей системы", тот требует прав администратора и не нужен.
3. В открывшемся окне **"Переменные среды"**, в верхней части ("Переменные среды пользователя для ВашеИмя"), нажмите **"Создать..."** (*"New..."*).
4. В маленьком окне заполните:
   - **Имя переменной:** `JEV_API_KEY`
   - **Значение переменной:** вставьте ключ (Ctrl+V)
5. Нажмите **ОК**.
6. Для языка ответов Jev — повторите шаги 3–5: имя `JEV_LANGUAGE`, значение `ru`.
7. При доступе через OpenRouter — так же добавьте `OPENROUTER_API_KEY` со значением ключа `sk-or-v1-...`.
8. Нажмите **ОК** на всех оставшихся окнах ("Переменные среды", "Свойства системы"), чтобы закрыть их и применить изменения.
9. **Закройте и заново откройте** все открытые окна PowerShell/терминала/VS Code — переменная подхватится только в новых окнах.

Проверка — новое окно PowerShell:
```powershell
$env:JEV_API_KEY
```
Должен напечататься ключ. Пусто — либо не сохранилось на шаге 5, либо окно не новое.

**Способ 2: через PowerShell командой (без GUI)**
...
После каждой строки — Enter; PowerShell ответит `SUCCESS: Specified value was saved.` Закройте это окно, откройте новое, проверьте так же через `$env:JEV_API_KEY`.

**`setx` vs `$env:` — не перепутать:**
- `setx JEV_API_KEY "..."` — сохраняет **постоянно** (переживёт перезагрузку), но требует нового окна, чтобы подхватиться.
- `$env:JEV_API_KEY = "..."` — работает **сразу**, но только в текущем окне PowerShell; закрыли окно — значение пропало. Годится для разового теста, не для постоянной работы.

**Проверка всей цепочки целиком** (после того как `$env:JEV_API_KEY` что-то печатает в новом окне):
```powershell
& "$HOME\.claude\scripts\ojc\jev-route.ps1" "rename a variable in utils.ts"
```
JSON с полем `choice` в ответе — всё настроено верно. Ошибка авторизации — либо ключ не сохранился (переоткройте окно), либо скопировался с лишним пробелом/переносом строки (скопируйте ключ заново, без соседних символов).

**Чего не делать:**
- Не вставляйте ключ в файлы репозитория (`CLAUDE.md`, скрипты и т.п.) — они в git, ключ попадёт в историю коммитов навсегда, даже если потом удалить файл.
- Не отправляйте ключ в чат ИИ-ассистенту или куда-либо ещё текстом "для проверки" — если это случилось, зайдите на thejevai.com/openrouter.ai и **перевыпустите ключ** (старый станет недействительным).
- Не храните ключ в обычном текстовом файле на рабочем столе "на всякий случай" — переменная окружения через способ 1 достаточно безопасна для личной машины и удобнее.
